...
public class RedisSessionManager extends ManagerBase implements Lifecycle {
  ...
  public void add(Session session) {
    try {
      sessions.put(session.getIdInternal(),session);//add by huenbin
      save(session);
  ...
  public void save(Session session) throws IOException {
    ...
    try {
      ...
      // fix a bug  NullPointerException see github source
      if (toBoolean(sessionIsDirty) || !toBoolean(currentSessionIsPersisted.get())) {
  ...
  /**
   *  convert Boolean to boolean  when null return false
   * @param booleanValue Boolean
   * @return
   */
  public boolean toBoolean(Boolean booleanValue){
  ...
  public void processExpires() {
    // We are going to use Redis's ability to expire keys for session expiration.
     super.processExpires();//add supper method
    // Do nothing.
  }
